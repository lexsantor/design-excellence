...
| AUDIT (stage) | moderate | Task Mode ∈ {REDESIGN, AUDIT, DISCOVER} — not CRITIQUE mode, whose evaluation of the existing implementation is its own CRITIQUE-as-deliverable stage (§2 CRITIQUE row; Phase 9.2C correction — CRITIQUE was previously also listed here, contradicting that row). When a bounded Pipeline Profile is active, its sequence alone decides: a Task Mode listed here does not activate the AUDIT stage if that sequence omits it (§1, "Requested deliverable bounds the Pipeline Profile"; Phase 9.2F) |
...
| BUILD | task-dependent | any mode except pure CRITIQUE/AUDIT; not reached by a bounded Pipeline Profile that ends before it (§1, "Requested deliverable bounds the Pipeline Profile") |
| VALIDATE | scales with scope | always — but only the gates relevant to what was touched |
| CRITIQUE | cheap floor always **except Task Mode ∈ {AUDIT, DISCOVER}**; escalation conditional | see `references/critique-protocol.md` — AUDIT mode does not run a self-critique-of-own-output pass, because there is no newly generated output to critique; the AUDIT-stage read-only findings pass *is* the deliverable (Phase 4 correction — this exemption previously lived only in `critique-protocol.md`'s prose, not stated here). DISCOVER is exempt for the same reason: its deliverable is also the read-only AUDIT-stage findings pass, and its §2 row lists no CRITIQUE stage (Phase 9.2C correction — previously only AUDIT was named here). **`CRITIQUE(floor)`**, wherever it appears in a sequence (§1 bounded profiles, §2 rows), means the Level 1 floor is the mandatory minimum, not the exclusive depth: Level 2 still runs whenever `critique-protocol.md` independently triggers it. Only **`CRITIQUE(floor only)`** (§2 POLISH row) caps the stage at Level 1 (Phase 9.2I clarification) |
...
Do not invent a concept the brief doesn't supply — a HYPOTHESIS must be a real, checkable association, never a fabricated one. A HYPOTHESIS may still inform ordinary creative exploration during DIRECT/EXPLORE (below), exactly as any other undeclared creative idea could, but it must never be logged or treated as detected concept evidence, and it does not activate `references/rule-catalog/layout-interaction.md` LAYOUT-009's mandatory structural-translation requirement (see LAYOUT-009's own `applicability`) or the Level 1/Level 2 structural-test check (`references/critique-protocol.md`). A HYPOTHESIS may inform ordinary creative exploration, including mood, tone, palette, rhythm, or other indirect aesthetic choices, but it must not by itself justify a literal, recognizable icon, logomark, or motif representing the hypothesis itself — that requires promoting the concept to DETECTED / EVIDENCED first, using the same bright-line test already stated above. This detection never activates where DEFINE itself doesn't run — component/section POLISH has no path here, per the Hard rule above — and it does not change how Register/Genre/Dials (§4) are classified.

**Creative exploration inside DIRECT (Phase 6 correction):** for page/multi-page-site scope BUILD/REDESIGN only — never for section/component POLISH, which has no path into DIRECT at all, per the Hard rule above — DIRECT states 2 candidate directions (or the number the user asks for) before committing to one, each a one-line conceptual sketch, not a full design or mockup — unless the requested deliverable is directions only, in which case no commit happens (see "Requested deliverable bounds the Pipeline Profile" below). See §5 for the distinctness and evidence-cross-check requirements on what gets logged as Rejected Directions. This is a lightweight comparison step inside the existing DIRECT stage, not a new pipeline stage, and it does not create a style catalog. When DEFINE recorded a DETECTED / EVIDENCED concept (above) — never for a HYPOTHESIS-only outcome — the two candidates must also offer different structural interpretations of it, per `references/rule-catalog/layout-interaction.md` LAYOUT-009 (Phase 6.3 addition) — the organizing surface each candidate uses is recorded alongside the existing distinctness dimensions (§5), and this translation happens as part of choosing between the two candidates, never as a decorative addition applied after one is already picked.

**Visual Reference Evidence inside EXPLORE (Phase 6.4 addition):** when `references/visual-references.md` or `references/principle-pool.md` (§7, loaded together as one combined source) is loaded and yields at least one entry compatible with the current Register/Genre, EXPLORE considers whether an abstracted reference principle (`layout-interaction.md` LAYOUT-010) meaningfully broadens either candidate beyond what the brief + Genre's own default posture would generate alone. This is a judgment, not a quota: a reference may inform zero, one, or both candidates, and "considered, then judged unnecessary" is as valid an outcome as "considered, then adopted" — never require a candidate to use a reference merely because one was available. When a reference does inform a candidate, its id and abstracted principle are logged alongside that candidate's other distinguishing dimensions in Rejected Directions (§5), the same mechanism LAYOUT-009 already uses, not a new one. Independent of Concept → Structure — a concept determines what the brief's meaning might require; a reference can inform how a candidate expresses that (or express something unrelated to any concept); neither mechanism depends on the other being present.

**Requested deliverable bounds the Pipeline Profile (Phase 9.2C clarification):** the user's requested deliverable bounds the Pipeline Profile. For page/multi-page-site BUILD/REDESIGN, the Task Mode and its §2 row are unchanged; when the request asks for less than the full pipeline, the run ends early at the stage that delivers it. This is a **bounded Pipeline Profile** — a routing qualifier on an existing Task Mode, never a Task Mode of its own (§2). Three cases, read from what the request asks for, not from its Task Mode:

- **Directions only** — the request asks to explore, generate, compare, or propose directions, and does not ask to choose, prepare, or implement one. UNDERSTAND→INSPECT→DEFINE→DIRECT/EXPLORE→VALIDATE/CRITIQUE(floor, scoped to the deliverable, §1)→SHIP(report). Generate the requested number of candidates, compare them, return them as the deliverable, name unresolved decisions, and stop. No commit; nothing written to Rejected Directions or Fingerprint History (§5, §6 — both record a committed direction, and none exists); no implementation files and no `.design/context.md` change implying a committed direction. The distinguishing dimensions §5 would log (including `LAYOUT-009` surfaces and `LAYOUT-010` references) go in the exploration output instead.
- **Choose / prepare** — the request asks to choose, commit to, or prepare a direction for implementation. UNDERSTAND→INSPECT→DEFINE→DIRECT/EXPLORE→commit→SHAPE→DESIGN (each where its own trigger applies)→VALIDATE/CRITIQUE(floor, scoped to the deliverable, §1)→SHIP(spec). Commit to one candidate, log the rejected ones per §5, record the prepared direction in `.design/` (§5), and stop before BUILD. "Prepare for implementation" does not mean "implement". Every existing-project safety gate still applies (`existing-project-safety.md`).
- **Implement** — the request explicitly asks to implement, build, code, or ship. The full BUILD or REDESIGN row (§2), unchanged.

The sequence listed for a bounded case is the complete set of stages for that run. A stage that the underlying Task Mode's §2 row would otherwise activate, but which that sequence does not list, does not run. This holds wherever the stage would fall in the §1 order, before or after the endpoint. The Task Mode itself does not change, and no P1 gate is suppressed (§3, "P2 and pipeline extent") (Phase 9.2E clarification). This rule governs which stages run, not how deeply a listed stage runs: if the sequence omits CRITIQUE, CRITIQUE does not run at all; if it lists `CRITIQUE(floor)`, Level 1 is guaranteed and `critique-protocol.md` alone decides whether Level 2 also runs (§1 CRITIQUE row). Level 2 is an escalation inside the CRITIQUE stage, not a stage, so it is never removed for being "not listed" (Phase 9.2I clarification).
...
| REDESIGN | full pipeline, INSPECT+AUDIT-stage both heavy, `existing-project-safety.md` mandatory, CRITIQUE escalates to Level 2 by default | — when unbounded; with a bounded Pipeline Profile, every stage its §1 sequence does not list, AUDIT-stage included (Phase 9.2F) |
...
State the classification back in one line before proceeding: *"Reading this as: [scope] [task mode] on a [project state] project."* This is cheap, catches misrouting immediately, and satisfies the disclosure requirement in §3. When the requested deliverable ends the run before the Task Mode's normal endpoint (§1, "Requested deliverable bounds the Pipeline Profile"), keep the Task Mode above and append the qualifier: *"…on a [project state] project; bounded Pipeline Profile: ends after [DIRECT | DESIGN]."* Do not coin a mode name for it (Phase 9.2C clarification).
...
**P2 and pipeline extent (Phase 9.2C clarification):** an explicit user instruction may narrow the requested deliverable and therefore bound the Pipeline Profile (§1), unless a P1 item requires otherwise. It only narrows scope and deliverable. It never skips a P1 gate, never bypasses the Replace confirmation in `existing-project-safety.md` §2, never authorizes fabricated product facts, and never authorizes writes outside the requested scope.
...
For section-scope BUILD on an already-directed page (§1 section-scope disambiguation), the page-level distinctness check above is skipped: the new section inherits that page's macrostructure, typography pairing, color-anchor, density band and motion tier by design, so the 2-of-5 test would push it away from its own page. Fingerprint History may still be updated to record the resulting section design state. Page-level checking is unchanged (Phase 9.2C clarification).
...
| T3 | `references/principle-pool.md` | Same trigger as `visual-references.md`: DIRECT/DESIGN stage for a new visual system **only**, never CRITIQUE/AUDIT. Loads together with `visual-references.md`, never on its own, and is consumed only through LAYOUT-010 as one combined source with it, not a new load event. Holds accepted, externally curated, identity-stripped design principles only, under its own governance header (ceiling, freshness rule); ids are opaque, the file never points outside itself, and nothing outside this repository is read at runtime |
| T4 | `references/gesture-physics.md`, `references/stacks/*.md`, `references/reicon.md`, `references/kinetics.md`, `references/smoothui.md` | gesture-physics only if drag/gesture interaction is actually in scope; a stack file only if that stack is detected/declared — none ship by default (see below); `reicon.md` only when new iconography is genuinely needed and no existing project icon system or explicit user instruction already covers it — see `reicon.md` for the full precedence model; `kinetics.md` only when a motion/microinteraction implementation decision is already in scope that a plain CSS transition does not already satisfy (motion need already established by `motion.md`/`MOTION_INTENSITY`) and no existing project motion system or explicit user instruction already covers it — see `kinetics.md` for the full precedence model; `smoothui.md` only when a component or UI pattern has already been established as necessary and implementation is genuinely in scope, and no existing project component system or explicit user instruction already covers it — see `smoothui.md` for the full precedence model |
