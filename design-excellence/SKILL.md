...
| T4 | `references/gesture-physics.md`, `references/stacks/*.md`, `references/reicon.md` | gesture-physics only if drag/gesture interaction is actually in scope; a stack file only if that stack is detected/declared — none ship by default (see below); `reicon.md` only when new iconography is genuinely needed and no existing project icon system or explicit user instruction already covers it — see `reicon.md` for the full precedence model |
