...
### A11Y-009 — Essential content must not depend on JavaScript to exist or become visible
- **category:** accessibility · **layer:** P5 (UX/usability — a critical, real defect, but not literally one of the four items on the `SKILL.md` §3 enumerated P1 list; kept P5 rather than expanding that list, consistent with the Phase 4 correction's discipline on this file)
- **principle:** essential user-facing content and essential interactive controls (primary page content, navigation, and navigation's own toggle) must render visible and operable in HTML+CSS alone, before any JavaScript executes. JavaScript may enhance presentation, animate, or add richer interaction, but must never be the only mechanism that makes essential content exist or become visible. Do not default essential content to `opacity: 0`, `visibility: hidden`, an off-screen position, or a "hidden"/not-yet-revealed class that only a script removes. Prefer: HTML visible by default → CSS presentation → JavaScript enhancement. Avoid: HTML → hidden-by-default CSS state → JavaScript required to reveal it.
- **severity:** critical
- **applicability:** universal for essential content and essential interaction. Does **not** apply to legitimate hidden-by-design UI: closed accordions/disclosures/modals/drawers whose collapsed state is represented accessibly and operably, decorative or non-essential reveal choreography, lazy-loaded below-the-fold media with a valid fallback, or interaction-only UI intentionally unavailable until its trigger exists. A scroll-reveal or load-in animation on non-essential content stays governed by `MOTION-003`/`MOTION_INTENSITY`, not this rule — the failure this rule targets is essential content becoming permanently inaccessible if JavaScript never runs, not the presence of a reveal animation itself.
- **exceptions:** none for essential content/navigation; see applicability for what counts as non-essential.
- **evidence:** Phase 7 cross-project benchmark, Test 01 (MOVA) — hero content and the mobile nav toggle were both invisible and inert without JavaScript, with no fallback; missed by Level 1 critique and mechanical VALIDATE, caught only by a Level 2 reviewer manually tracing the JS-reveal chain. The causal shape (opacity-0-by-default plus a JS-only reveal class) is a common pattern in exactly the load/scroll-reveal motion this skill's brand-led builds reach for.
- **freshness:** status: permanent.
- **validation:** where rendered-browser capability is available, load the critical pages/components with JavaScript disabled (or simulate JS unavailability) and verify essential content and essential navigation are still visible and operable. Where rendered-browser capability is unavailable, inspect source for the failure shape directly: essential content or its trigger defaulting to `opacity: 0`/`visibility: hidden`/an off-screen position/a not-yet-revealed class, with no CSS-only visible state and no `<noscript>` or equivalent fallback.
- **remediation:** make the default CSS state visible and operable; move the enhancement (animation, staged reveal, richer interaction) to run *on top of* that default, not to gate it — e.g. animate an already-visible element's `transform`/`opacity` for polish, but never gate its baseline visibility behind a script-added class.
