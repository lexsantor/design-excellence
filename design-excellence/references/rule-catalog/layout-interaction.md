...
- **freshness:** status: permanent (the mechanism); entries carry their own freshness fields (`last_verified_date`, `review_interval_days`) in `visual-references.md` and `principle-pool.md`; each file states its staleness rule in its header.
